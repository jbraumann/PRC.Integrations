...
| `solver` | `string` | Kinematics solver relative to `PRC.Library.Solvers` (`"KUKA.KUKA_6DOF"`, `"KUKA.KUKA_6DOF_Offset"`, `"KUKA.KUKA_7DOF"`, `"ABB.ABB_6DOF"`, `"ABB.ABB_6DOF_Offset"`, `"UR.UR_6DOF"`, …) — the value `CustomRobot.kinematics_solver_class` takes, so a client can build a custom robot with a preset's kinematics. |
...
| `online` | `bool` | `true` when the driver executes tasks on a connected controller (`ABB.ABB_RWS_Driver`, `KUKA.KSS_MXA_Driver`, `KUKA.KSS_IOB_Driver`, `UR.UR_RT_Driver`, `NEURA.NEURA_RT_Driver`, `KUKA.KUKA_Sunrise_Driver` — the last one only in its online execution modes); `false` for drivers that only generate a program file. |
| `run_state_variable` | `string` | Name of the driver variable (`UpdateVariable` / `GetRobotData`) that carries the controller's execution state while a task runs: `"ABB RWS State"`, `"mxA State"`, `"UR State"`, `"NEURA State"`, `"Sunrise State"`. Empty when the driver cannot report execution. |
| `busy_values` | `repeated string` | Values (or prefixes) of `run_state_variable` that mean *a program is running* — `["Running"]` for ABB RWS, `["Streaming"]` for mxA and UR, `["Streaming", "Following target"]` for NEURA, `["Running program", "Following target"]` for Sunrise (in program-file mode the variable reads "Program file"). Any other value means the controller is idle. With `run_state_variable` this lets a client wait for a task to finish without vendor knowledge. |
...
**KUKA** (60 models):
`KUKA.KUKA_KR610R11002`, `KUKA.KUKA_KR610R9002`, `KUKA.KUKA_KR10R1420`, `KUKA.KUKA_KR120R1800`, `KUKA.KUKA_KR210R31002`, `KUKA.KUKA_KR3060`, `KUKA.KUKA_KR6R18402`, `KUKA.KUKA_KR8R1620`, `KUKA.KUKA_KR50R2500`, `KUKA.KUKA_KR1000`, `KUKA.KUKA_KR120R3900K`, `KUKA.KUKA_KR210KR240R2700`, `KUKA.KUKA_KR3R540`, `KUKA.KUKA_KR4R600`, `KUKA.KUKA_KR5arcHW`, `KUKA.KUKA_KR600R2830`, `KUKA.KUKA_KR480R3330`, `KUKA.KUKA_KR3605002`, `KUKA.KUKA_KR100120P2`, and many more — including the Agilus-3 ultra `KUKA.KUKA_KR10R11003` / `KUKA.KUKA_KR13R9003` (new in 1.727), the LBR iisy `KUKA.KUKA_LBR3R760`, the LBR iico `KUKA.KUKA_LBR12R1260` (offset wrist, `KUKA_6DOF_Offset` solver), the seven-axis **LBR iiwa** `KUKA.KUKA_LBR7R800` / `KUKA.KUKA_LBR14R820` (`KUKA_7DOF` solver, Sunrise driver only), and the Fortec-2 ultra `KUKA.KUKA_KR800R28002` (new in 1.735).

**Universal Robots:** `UR.UR_3` (UR3), `UR.UR_3e` (UR3e), `UR.UR_5` (UR5), `UR.UR_57e` (UR5e/7e), `UR.UR_10e`, `UR.UR_20`

**ABB:** `ABB.ABB_IRB120`, `ABB.ABB_IRB140`, `ABB.ABB_IRB6620`, `ABB.ABB_IRB6700_150_320`, and the GoFa cobots `ABB.ABB_CRB15000_5_095`, `ABB.ABB_CRB15000_10_152`, `ABB.ABB_CRB15000_12_127` (new in 1.735; offset wrist, `ABB.ABB_6DOF_Offset` solver)
...
| `KUKA.KUKA_Sunrise_Driver` | `OutputFormat` | `"Sunrise XML (PRC Java library)"` (**default**) or `"Sunrise Workbench Frames"` | Selects the program file: the version-3 XML the PRC Sunrise Java library executes (save as **`.xml`**), or a Sunrise Workbench `RoboticsAPIData` frame file (LIN and PTP targets with their redundancy data) to import into Workbench. |
| `KUKA.KUKA_Sunrise_Driver` | `ExecutionMode`, `RobotIP`, `RobotPort` | `"Program File"` (**default**) / `"Online: Run Program"` / `"Online: Follow Target"`; controller IP; `30000` | The online modes connect over TCP to the `PRC_RunOnline` application of the Java library (the controller is the server; PRC reconnects on its own). *Run Program* sends every executed task as a complete Sunrise XML program that replaces the running one; *Follow Target* sends the task's last PTP or axis target (with its simulated posture and joint solution) for the robot's SmartServo motion to follow in joint space — no priming motion; a task ending with a LIN, circular or spline target is refused. The controller's `STATE`/`PROGRAM`/`LOG` replies become the live robot state and the `Sunrise State` / `Sunrise Link` / `Sunrise Progress` / `Sunrise Program` / `Sunrise Message` variables; `Sunrise State` is `Running program` / `Following target` while busy. While the state streams, `GetSimulatedRobotState` does not move the live robot (the realtime-driver rule). `Run` false (the well-known variable) sends `STOP` — honoured within a state period without a task; a stop requested while disconnected is sent on the next connection, and an explicit `Run` true replays the last task (a simulate-only one promoted to execute). |
